...
class Payment < ActiveRecord::Base
  ...
  #Creates credit card object to be used for payment
  def credit_card
  ...
  #Checks validity of credit card and charges the given payment amount to the card
  def process
    if valid_card
      ...
      if response.success?
        ...
        if !transaction.success?
          errors.add(:base, `#{credit_card.errors.full_messages.join('. ')}`) and return
          false
        end
